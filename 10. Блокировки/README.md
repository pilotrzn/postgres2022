...
Попробовав выполнить такой вариант, когда в обной транзакции ввожу "update testtable set value=value+100;" и потом подобнй UPDATE во второй транзакции полчил зависание второй транзакции. Выполнив запрос к нашему представлению locks_v в первой транзакции, получил информацию что процесс держит EXclusiveLock, что не позволяет выполнять другую транзакцию, пока не завершится блокирующая.